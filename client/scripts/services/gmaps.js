                        ...
                        if (status == google.maps.GeocoderStatus.OK) {
                            ...
                            me.directionsService.route(request, function (result, status) {
                                ...
                                if (status == google.maps.DirectionsStatus.OK) {
                                    ...
                                    $rootScope.infoRouter = 'Ước lượng: ' + routes.legs[0].distance.text + ' - ' + $filter('toCurrency')(Math.round(routes.legs[0].distance.value / 1000) * 12000) + ' KSH';
                                   ...
                                   /* var animatedMarker = L.animatedMarker(me.RoutePolyline.getLatLngs(), {
                                    ...
                                    $rootScope.$apply(function () {
                                        ...
                                        if (Math.round(routes.legs[0].distance.value / 1000) * 12000 > 0) {
                                            ...
                                            if (!$rootScope.status.hasRouter) {
                                                $rootScope.pageTitleCalu = 'Ước lượng: ' + routes.legs[0].distance.text + ' - ' + $filter('toCurrency')(Math.round(routes.legs[0].distance.value / 1000) * 12000) + ' KSH';
                                            }
