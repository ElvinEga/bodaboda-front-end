...
angular.module('itaxiManagerApp')
    .controller('detailtDrivingCtrl', ['$scope', '$logger', '$timeout', '$fetchData', 'appDataStore' , '$baseModel', '$restful', '$http', '$state', '$stateParams', '$upload',
            ...
            $scope.uploads = function () {
                ...
                for (var i = 0; i < files.length; i++) {
                    ...
                    $scope.upload = $upload.upload({
                        url: 'https://bodaboda1.herokuapp.com/upload', // upload.php script, node.js route, or servlet url
                        method: 'POST', // or 'PUT',
        ...
        $scope.update = function (item) {
            ...
            itemSave.save(function (err,Lỗi cập nhật result) {
                if (!err) {
                    toastr.info('Updated Bodaboda Information Successfully!');
                    appDataStore.Drivings.update(result[0]);
                    $scope.enabale = true;
                    $scope.driver = result[0];
                } else {
                    toastr.error('Update Error Bodaboda Infornation');
                }
            })
        };
